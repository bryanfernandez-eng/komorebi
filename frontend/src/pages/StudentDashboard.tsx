...
function scoreColor(score: number) {
  if (score > 70) return '#c06060';
  if (score > 40) return '#cba052';
  return '#7eb88a';
}
  ...
  return (
    <div className="min-h-screen bg-[#0c0c0c] p-6 font-sans text-[#e8e8e8]">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {/* Header */}
        <div>
          <h1 className="text-xl font-semibold mb-1">Your Wellness Insights</h1>
        ...
        {/* No data state */}
        {!hasScore && (
          <div className="bg-[#161616] border border-[#252525] rounded-2xl p-8 text-center">
            <div className="text-3xl mb-3">📋</div>
            <p className="text-[15px] font-medium text-[#ccc] mb-1">No assessment yet</p>
            <p className="text-[13px] text-[#555]">Submit a check-in and the AI agents will analyze your data — results appear here within seconds.</p>
          </div>
        )}
        ...
        {hasScore && (
          <div
            ...
            style={{
              backgroundColor: finalScore > 70 ? '#1a1515' : finalScore > 40 ? '#1a1710' : '#141a15',
              border: `1px solid ${finalScore > 70 ? '#3a1f1f' : finalScore > 40 ? '#3a3010' : '#1e3323'}`,
            }}
          >
            {/* Header row */}
            <div className="flex items-start justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-[14px] font-semibold text-[#e8e8e8]">AI Risk Assessment</h2>
                  <p className="text-[11px] text-[#666] mt-0.5">Analyzed by 4 AI agents after your last check-in</p>
                </div>
              </div>
              {/* Overall verdict */}
              <div
              ...
              >
                ...
                <div className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: `${color}cc` }}>
            ...
            <div className="space-y-4 mb-5">
              <div>
                <div className="flex justify-between text-[12px] mb-2">
                  <div>
                    <span className="text-[#aaa] font-medium">Risk score</span>
                    <span className="text-[#555] ml-2">Signal Agent ran 3 loops — mood, sleep, stress + emotion analysis</span>
                  </div>
                  <span className="text-[#ccc] font-semibold flex-shrink-0 ml-2">{latestAlert.risk_score}/100</span>
                </div>
                <ScoreBar value={latestAlert.risk_score ?? 0} color="#8a8dc4" />
              </div>

              <div>
                <div className="flex justify-between text-[12px] mb-2">
                  <div>
                    <span className="text-[#aaa] font-medium">Academic stress multiplier</span>
                    <span className="text-[#555] ml-2">Context Agent read the USF academic calendar</span>
                  </div>
                  <span className="text-[#ccc] font-semibold flex-shrink-0 ml-2">+{latestAlert.stress_multiplier}</span>
                </div>
                <ScoreBar value={latestAlert.stress_multiplier ?? 0} max={25} color="#cba052" />
              </div>

              <div className="pt-3 border-t border-[#ffffff10]">
                <div className="flex justify-between text-[13px] mb-2">
                  <span className="font-semibold text-[#ccc]">Final score</span>
            ...
            {latestAlert.minimization_detected && (
              <div className="mt-4 flex items-start gap-2 p-3 rounded-xl bg-[#1f1a11] border border-[#3b301c]">
                <AlertCircle className="w-3.5 h-3.5 text-[#cba052] mt-0.5 flex-shrink-0" />
                <p className="text-[12px] text-[#a88a50] leading-relaxed">
                  <span className="font-semibold">Emotion analysis flag:</span> Your text entries suggest more stress than your numerical scores indicate. The AI detected possible minimisation and adjusted your risk score upward.
                </p>
              </div>
            )}
          </div>
        ...
        {trendContext && (
          <div className="bg-[#161616] border border-[#252525] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Users className="w-4 h-4 text-[#8a8dc4]" />
                <h2 className="text-[13px] font-semibold text-[#ccc]">Campus Context</h2>
              </div>
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#1e1e3a] border border-[#2e2e5a] text-[#8a8dc4]">
            ...
            <p className="text-[14px] leading-relaxed text-[#aaa]">{trendContext}</p>
          </div>
        )}
          ...
          <div className="bg-[#161616] border border-[#252525] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-[13px] font-semibold text-[#ccc]">Message from the AI</h2>
              </div>
              {latestActionAlert.message_es && (
                <button
                  ...
                  style={{ color: '#8a8dc4', backgroundColor: '#1e1e3a', border: '1px solid #2e2e5a' }}
                >
        ...
        {latestPrediction && (
          <div className="bg-[#161616] border border-[#252525] rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles className="w-4 h-4 text-[#7eb88a]" />
              <h2 className="text-[13px] font-semibold text-[#ccc]">7-Day Forecast</h2>
            </div>
            <p className="text-[13px] text-[#555] mb-2">Gemini's prediction based on your recent trend and upcoming academic events.</p>
            <p className="text-[15px] leading-relaxed text-[#ccc]">{latestPrediction}</p>
          </div>
        )}
        ...
        <div className="bg-[#161616] border border-[#252525] rounded-2xl p-5">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-[13px] font-semibold text-[#ccc]">7-Day Check-in History</h2>
            </div>
          ...
          <p className="text-[12px] text-[#444] mb-5">Raw data from your daily check-ins — what the Signal Agent sees.</p>
            ...
            {history.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={history} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                  ...
                  <XAxis
                    dataKey="date" stroke="#333" fontSize={11}
                    tickFormatter={(val) => { const p = val.split('-'); return p.length === 3 ? `${p[1]}/${p[2]}` : val; }}
                    tickMargin={10} axisLine={false} tickLine={false}
                  />
                  <YAxis domain={[1, 10]} stroke="#333" fontSize={11} tickCount={4} axisLine={false} tickLine={false} />
                  <Tooltip
                  ...
                  />
                  <Line type="monotone" dataKey="mood"   name="Mood"   stroke="#7eb88a" strokeWidth={2} dot={{ fill: '#0c0c0c', stroke: '#7eb88a', strokeWidth: 2, r: 3 }} activeDot={{ r: 5 }} />
                  <Line type="monotone" dataKey="sleep"  name="Sleep"  stroke="#4a8ba8" strokeWidth={2} dot={{ fill: '#0c0c0c', stroke: '#4a8ba8', strokeWidth: 2, r: 3 }} activeDot={{ r: 5 }} />
                  <Line type="monotone" dataKey="stress" name="Stress" stroke="#c06060" strokeWidth={2} dot={{ fill: '#0c0c0c', stroke: '#c06060', strokeWidth: 2, r: 3 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
